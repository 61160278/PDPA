...
<body>
    <div id="wrapper">
        <?php 
            include "menu.php";
            $sql_emp = "SELECT * From authorize_international_module WHERE aut_int_emp_id = ".$_SESSION["emp_id"]." ";
            $result_emp = mysqli_query($condbmc, $sql_emp);
            $row_emp = mysqli_fetch_array($result_emp);

            if($row_emp['aut_int_role_id'] == 1){
                $sql_command = "SELECT * From borrow_detail WHERE borrow_detail_status = 1   ";			
            }else if($row_emp['aut_int_role_id'] == 2){
                $sql_command = "SELECT * From borrow_detail WHERE borrow_detail_status = 2   ";
            }else if($row_emp['aut_int_role_id'] == 3){
                $sql_command = "SELECT * From borrow_detail WHERE borrow_detail_status = 3  ";
            }else if($row_emp['aut_int_role_id'] == 4){
                $sql_command = "SELECT * From borrow_detail WHERE borrow_detail_status = 4   ";
            }
        ?>
                            ...
                            <table class="table table-striped table-bordered table-hover dataTables-example" >
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $sql_name = "SELECT * From employee WHERE Emp_ID ";
